...
pub const DrawCallback_ResetRenderState: usize = ~@as(usize, 0);
    ...
    return extern struct {
        ...
        pub fn clone(self: @This()) @This() {
            ...
            if (self.Size != 0) {
                ...
                @memcpy(@as([*]u8, @ptrCast(cloned.Data.?)), @as([8]*const u8, @ptrCast(self.Data.?))[0..self.Size * @sizeOf(T)]);
            }
            return cloned;
        }
        ...
        pub fn copy(self: *@This(), other: @This()) void {
            ...
            if (other.Size != 0) {
                ...
                @memcpy(@as([*]u8, @ptrCast(self.Data.?)), @as([*]const u8, @ptrCast(other.Data.?))[0..other.Size * @sizeOf(T)]);
            }
        }
        ...
        pub fn from_slice(slice: []const T) @This() {
            ...
            if (slice.len != 0) {
                ...
                @memcpy(@as([*]u8, @ptrCast(result.Data.?)), @as([*]const u8, @ptrCast(slice.ptr))[0..slice.len * @sizeOf(T)]);
            }
            return result;
        }
        ...
        pub fn reserve(self: *@This(), new_capacity: u32) void {
            ...
            const new_data: ?[*]T = @ptrCast(@alignCast(raw.igMemAlloc(new_capacity * @sizeOf(T))));
            if (self.Data) |sd| {
                if (self.Size != 0) {
                    @memcpy(@as([*]u8, @ptrCast(new_data.?)), @as([*]const u8, @ptrCast(sd))[0..self.Size * @sizeOf(T)]);
                }
        ...
        pub fn reserve_discard(self: *@This(), new_capacity: u32) void {
            ...
            self.Data = @ptrCast(@alignCast(raw.igMemAlloc(new_capacity * @sizeOf(T))));
            self.Capacity = new_capacity;
        }
...
fn imguiZigAlloc(_: *anyopaque, len: usize, ptr_align: u29, len_align: u29, ret_addr: usize) std.mem.Allocator.Error![]u8 {
    ...
    return @as([*]u8, @ptrCast(raw.igMemAlloc(len) orelse return error.OutOfMemory))[0..len];
}
